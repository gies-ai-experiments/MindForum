...
/**
 * Owner-authed GitHub preview for the dashboard settings flow. The
 * participant-side preview (`/api/room/[id]/context/github/preview`) requires a
 * `mindforum_pid_<roomId>` cookie, which a creator who has never joined the
 * room doesn't have — so a fresh owner's Preview failed with `not_joined` even
 * though Attach (this dashboard path) succeeds. Gate on room ownership instead,
 * matching the sibling attach route.
 */
...
function githubErrorResponse(err: unknown): NextResponse {
  ...
  console.error("dashboard github context preview failed:", err);
