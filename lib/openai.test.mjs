...
test("web_search with a non-string query is treated as missing (not searched)", async () => {
  const searched = [];
  const gen = chatReplyStream([], files, "", {
    openai: fakeOpenAI([
      [toolCall("call_1", "web_search", JSON.stringify({ query: 42 }))],
      [content("Answered without searching.")],
    ]),
    webSearch: true,
    webSearchFn: async (q) => { searched.push(q); return "should not run"; },
  });
  let out = ""; for await (const t of gen) out += t;
  assert.deepEqual(searched, []); // non-string query → webSearchFn never called
  assert.equal(out, "Answered without searching.");
});

// --- generateSystemPrompt (non-streaming LLM helper) ---

import { generateSystemPrompt } from "./openai.ts";
import { MAX_SYSTEM_PROMPT_CHARS } from "./limits.ts";

// Non-streaming fake: one create() call returning a scripted completion.
function fakeCompletion(content) {
  const calls = [];
  const oa = {
    chat: { completions: { create: async (params) => {
      calls.push(params);
...
test("generateSystemPrompt returns trimmed model output and sends the description", async () => {
  const { oa, calls } = fakeCompletion("  You are a facilitator for four faculty.  ");
  const out = await generateSystemPrompt("help four faculty brainstorm", { openai: oa });
  assert.equal(out, "You are a facilitator for four faculty.");
  assert.equal(calls[0].messages[0].role, "system");
  assert.equal(calls[0].messages[1].role, "user");
  assert.equal(calls[0].messages[1].content, "help four faculty brainstorm");
});

test("generateSystemPrompt hard-caps output at MAX_SYSTEM_PROMPT_CHARS", async () => {
  const { oa } = fakeCompletion("x".repeat(MAX_SYSTEM_PROMPT_CHARS + 500));
  const out = await generateSystemPrompt("whatever", { openai: oa });
  assert.equal(out.length, MAX_SYSTEM_PROMPT_CHARS);
});

test("generateSystemPrompt returns '' when the model returns no content", async () => {
  const { oa } = fakeCompletion(null);
  const out = await generateSystemPrompt("whatever", { openai: oa });
  assert.equal(out, "");
});
