...
import {
  ...
  isMentionRemindersEnabled, setMentionRemindersEnabled,
} from "./store.ts";
...
test("toggle defaults ON; disabling cancels pending and blocks future arming", async () => {
  const r = await createRoom("mr-toggle-" + nanoid(6), "admin", "");
  const u1 = (await upsertParticipant(r.id, "Uno", "uno@x")).id;
  const u2 = (await upsertParticipant(r.id, "Dos", "dos@x")).id;
  // New rooms default to enabled.
  assert.equal(await isMentionRemindersEnabled(r.id), true);
  // Arm an overdue reminder, then disable -> it should be cancelled (skipped).
  const mid = nanoid(10);
  await appendMessage({ id: mid, roomId: r.id, authorId: u1, authorName: "Uno", content: "@dos ping", createdAt: Date.now(), kind: "chat" });
  await armMentionReminders({
    roomId: r.id, messageId: mid,
    createdAt: Date.now() - 2 * 60 * 60 * 1000,
    author: { id: u1, name: "Uno", email: "uno@x" },
    mentioned: [{ id: u2, name: "Dos" }],
    delayMs: 60 * 60 * 1000,
  });
  await setMentionRemindersEnabled(r.id, false);
  assert.equal(await isMentionRemindersEnabled(r.id), false);
  const claimed = await claimDueMentionReminders(500);
  assert.equal(claimed.filter((x) => x.roomId === r.id).length, 0); // pending was cancelled
  // Re-enabling flips the flag back.
  await setMentionRemindersEnabled(r.id, true);
  assert.equal(await isMentionRemindersEnabled(r.id), true);
});
