...
test("source type guard accepts only known source types", () => {
  assert.deepEqual(SOURCE_TYPES, ["uploaded", "github_repo", "web_url", "pasted_text"]);
  assert.equal(isSourceType("uploaded"), true);
  assert.equal(isSourceType("github_repo"), true);
  assert.equal(isSourceType("web_url"), true);
  assert.equal(isSourceType("pasted_text"), true);
...
test("source labels and warnings are explicit for v1 source types", () => {
  ...
  assert.equal(sourceLabel("pasted_text"), "Pasted text");
  assert.match(untrustedSourceWarning("github_repo"), /untrusted source material/);
  assert.match(untrustedSourceWarning("pasted_text"), /untrusted source material/);
});

test("source metadata validator handles pasted text", () => {
  assert.deepEqual(validateSourceMeta("pasted_text", { charCount: 42 }), { charCount: 42 });
  assert.equal(validateSourceMeta("pasted_text", { charCount: -1 }), null);
  assert.equal(validateSourceMeta("pasted_text", {}), null);
  assert.equal(validateSourceMeta("pasted_text", { charCount: "x" }), null);
});
