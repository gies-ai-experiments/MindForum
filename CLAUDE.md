...
### 2026-06-15
- Completed: **Shipped PR #25 (room invitations + creator-dashboard redesign + Entra ID SSO) and closed the #22 VPS cleanup.** Ran a **4-round `codex review --base main`** on the `test` branch → found & fixed **14 real bugs** (4 commits): R1 middleware Auth.js-v5 cookie mismatch (`authjs.session-token`; would've bounced every Entra user from `/dashboard/*`), owner-settings-link regression, Entra-aware sign-out, token self-invalidation, owner GitHub-preview auth; R2 anonymous-join rate-limit burn on shared IPs, invitation-cancel IDOR, archived-owner join, invite-link 404, Entra `next`; R3 `trustHost`, closed-room write guards, archived-invite accept (debunked codex's "start-command" P1 — `output:standalone` makes it inert on Azure); R4 Entra UPN fallback, attach-modal error visibility. Validated the Entra flow **end-to-end on the staging slot** via browser, then merged (`064fd80`). Configured prod SSO: set `NEXTAUTH_URL` + added the missing prod redirect URI (`mindforum.disruptionlab.illinois.edu/api/auth/callback/...`) to the Entra app reg (was absent → would've been `AADSTS50011`). Diagnosed a prod deploy "succeeds-but-running-app-not-updated" bug (stale build served) → Ash fixed it by zipping the Azure package (`a74b45c`); **verified Entra SSO live on prod** end-to-end (sign-out → Sign in with Illinois → settings sub-route renders under a real `authjs.session-token`). VPS deploy retired (`deploy.yml` deleted, `2c3df35`); refreshed CLAUDE.md deploy/#22 status (`74411a3`). Posted fix-notes on PR #25 + a deploy-pipeline handoff on #22.
- Next: **Decommission the VPS box** when ready (stop PM2 + remove repo/key — currently kept as a manual DNS rollback). (Prod DB cutover `postgres` → `mindforum` db is **DONE** — prod + staging slots both point at the `mindforum` db, data intact.) Still-pending side items: OpenAI monthly spend cap (defense-in-depth #2), faculty invitation for `ai-ethics-exercise`, the **overdue MSBAi four-week review** (was 2026-05-25). ⚠️ Rotate the `dlpostgresqladmin` Postgres password (it surfaced in this session's transcript via `az appsettings list`).
