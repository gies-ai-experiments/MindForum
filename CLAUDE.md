...
- [ ] **Verify creator-rooms v1 against Acceptance Checklist on the deployed VPS** — walk all 12 acceptance items on production now that auto-deploy has fired and v6/v7/v8 migrations have landed.
- [x] **Polls & Decisions v1** — `/poll` command with AI option draft, single-choice hidden-tally voting (5m/15m/1h/24h/manual), lazy expiry, automatic inclusion in project brief's new "Decisions & Votes" section. Branch `polls-and-decisions` off main (now rebased onto creator-rooms-v1 main, migration renumbered v6 → v9). Design at `docs/plans/2026-05-13-polls-and-decisions-design.md`, implementation plan at `docs/plans/2026-05-13-polls-and-decisions.md`.
