...
**[PR #26](https://github.com/gies-ai-experiments/MindForum/pull/26) — participant file removal + chat-coherent system note — opened 2026-06-18, assigned to Ash for review.** Branch `feat-file-removal` (2 commits, ~268 LOC): uploader can delete their own file via an in-room Delete button; owner/super-admin keeps the existing dashboard delete power; every delete atomically posts a `kind:"system"` chat note in a single Postgres tx (`deleteRoomFileWithSystemNote`) so future readers understand why later `@ai` replies stop referencing the doc. 2-round Codex Plan Reviewer (R1 REJECT → atomicity fix; R2 surfaced `Msg.kind` P2) + 1-round `codex review --base main` (P2 closed-room guard on uploader-delete, fixed in commit 2). Also fixes a pre-existing bug: the `file_removed` SSE was broadcast but never listened for, so owner dashboard deletes left a ghost file in everyone's in-room panel until reload. Acceptance walk pending on the Azure staging slot before merge.

Other side items still pending: OpenAI monthly spend cap (defense-in-depth #2), faculty invitation for `ai-ethics-exercise`, overdue MSBAi four-week review (was 2026-05-25), ⚠️ rotate the `dlpostgresqladmin` Postgres password (leaked in the 2026-06-15 transcript via `az appsettings list`).
...
### 2026-06-18
- Completed: **Opened [PR #26](https://github.com/gies-ai-experiments/MindForum/pull/26) — participant file removal + chat-coherent system note.** Branch `feat-file-removal` (2 commits, ~268 LOC). Designed via **2-round Codex Plan Reviewer**: R1 REJECT on a real P1 (file delete + system-note INSERT could get separated — `deleteRoomFile` is permanent, `appendMessage` can fail and isn't caught) → fixed by adding `deleteRoomFileWithSystemNote` (`lib/store.ts`) which runs both statements in a single `tx()` modeled on `hardDeleteRoom` — a 204 now guarantees both the file row is gone AND the chat note is persisted; audit log stays outside the tx (best-effort record, not a gate). R2 surfaced a P2 (the room page `Msg.kind` union was `"chat" | "brief"`, missing `"system"`) — folded in. **1-round `codex review --base main`** on the branch found one P2 (uploader-delete branch checked `archived_at` but missed `closed_at`, while the participant upload route checks both — a closed room is "session ended", participants shouldn't mutate file context or transcript) → fixed in commit 2. Implementation: two-branch auth in `app/api/room/[id]/files/[fileId]/route.ts` DELETE (owner/super-admin via `getActor`+`getRoomMeta` → any file; else `requireRoomParticipant` → own file only, 403 `not_your_file` if joined but not the uploader, 401 `not_joined` preserves cross-owner hiding); per-row Delete button in `app/room/[id]/page.tsx` Files panel shown only when `f.uploadedById === participantIdRef.current`; `SystemNoteView` renders system messages as a muted italic transcript line; `file_removed` SSE listener added (fixes a **pre-existing bug**: the route already broadcast `file_removed` but no listener existed, so owner dashboard deletes left a ghost file in everyone's in-room panel until reload); `ActivityFeed.tsx` `file.delete` case appends `(by owner|uploader|super-admin)`. No schema changes, no migration, no storage changes. `npx tsc --noEmit` clean; `npm test` 51/59 (the 8 failures are pre-existing poll-logic tests on `main`, verified by stashing). PR assigned to Ash (`ashcastelinocs124`) for review. Stored Ash's GitHub handle in project memory (`collaborators.md`) so future PRs don't re-derive it from `gh pr list`.
- Next: **Ash reviews PR #26** → acceptance walk on the Azure staging slot (participant upload → delete → file vanishes live for all → system note appears; owner dashboard delete → system note also fires; 403/401/410 curl checks; Activity feed shows `by uploader|owner`) → merge to `main` → auto-deploy to prod. Carry-forward side items: OpenAI monthly spend cap (defense-in-depth #2), faculty invitation for `ai-ethics-exercise`, the **overdue MSBAi four-week review** (was 2026-05-25), ⚠️ rotate the `dlpostgresqladmin` Postgres password (leaked in the 2026-06-15 transcript).
