...
- Next: **Decommission the VPS box** when ready (stop PM2 + remove repo/key — currently kept as a manual DNS rollback). Still-pending side items: finish the prod DB cutover (`postgres` → `mindforum` db — prod still runs on the default `postgres` db), OpenAI monthly spend cap (defense-in-depth #2), faculty invitation for `ai-ethics-exercise`, the **overdue MSBAi four-week review** (was 2026-05-25). ⚠️ Rotate the `dlpostgresqladmin` Postgres password (it surfaced in this session's transcript via `az appsettings list`).
